...
int ubi_io_is_bad(const struct ubi_device *ubi, int pnum)
{
	...
	ubi_transfer_peb(ubi, pnum, &ppnum, &ppeb_size);
	if (ubi->bad_allowed) {
		int ret;
	
		//printk("check pnum = %x, size = 0x%x, offs = %llx\n", pnum, ubi->peb_size, (loff_t)pnum * ubi->peb_size);	
		for(i=0; i<ubi->merged_peb_count; i++){
			//printk("check ppnum = %x, psize = 0x%x, poffs = %llx\n", ppnum, ppeb_size, (loff_t)ppnum * ppeb_size);	
			ret = mtd->block_isbad(mtd, (loff_t)ppnum * ppeb_size);
			if(ret != 0){
				//printk("found bad 0x%llx\n", (loff_t)ppnum * ppeb_size);
				break;
			}
			ppnum += 1;
		}
...
int ubi_io_mark_bad(const struct ubi_device *ubi, int pnum)
{
	...
	ubi_transfer_peb(ubi, pnum, &ppnum, &ppeb_size);
	//printk("mark pnum = %x, size = 0x%x, offs = %llx\n", pnum, ubi->peb_size, (loff_t)pnum * ubi->peb_size);	
	for(i=0; i<ubi->merged_peb_count; i++){
		//printk("mark ppum = %x, psize = 0x%x, poffs = %llx\n", pnum, ppeb_size, (loff_t)ppnum * ppeb_size);	
		err = mtd->block_markbad(mtd, (loff_t)ppnum * ppeb_size);
		if(err == 0){
			//printk("mark bad 0x%llx\n", (loff_t)ppnum * ppeb_size);
			error = 0;	
		}
		ppnum += 1;
	}
