		       ...
		       size_t *retlen, unsigned char *buf)
{
	...
	lnum = div_u64_rem(from, ((mtd->erasesize) * ubi->merged_peb_count), &offs);
#else
	...
	while (total_read) {
#if CONFIG_MTD_UBI_MERGE
		size_t to_read = ((mtd->erasesize) * ubi->merged_peb_count) - offs;
#else
			...
			size_t *retlen, const u_char *buf)
{
	...
	if (!(mtd->flags & MTD_WRITEABLE))
		return -EROFS;
#if CONFIG_MTD_UBI_MERGE
	lnum = div_u64_rem(to, ((mtd->erasesize) * ubi->merged_peb_count), &offs);
#else
	...
	while (total_written) {
#if CONFIG_MTD_UBI_MERGE
		size_t to_write = ((mtd->erasesize) * ubi->merged_peb_count) - offs;
#else
...
static int gluebi_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	struct gluebi_device *gluebi;
#if CONFIG_MTD_UBI_MERGE
	if (instr->addr < 0 || instr->addr > mtd->size - ((mtd->erasesize) * ubi->merged_peb_count))
#else
...
#if CONFIG_MTD_UBI_MERGE	
	instr->fail_addr = (long long)lnum * ((mtd->erasesize) * ubi->merged_peb_count);
#else
