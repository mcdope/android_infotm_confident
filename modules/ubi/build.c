...
static int io_init(struct ubi_device *ubi)
{
	...
	if((ubi->mtd->size >> ubi->mtd->erasesize_shift) > 4096)
		ubi->merged_peb_count = 0x400000 / ubi->mtd->erasesize;

	if(!ubi->merged_peb_count)
		ubi->merged_peb_count = 1;

	ubi->peb_size   = ubi->merged_peb_count * (ubi->mtd->erasesize);
	ubi->peb_count  = mtd_div_by_eb(ubi->mtd->size, ubi->mtd) / ubi->merged_peb_count;
#else
